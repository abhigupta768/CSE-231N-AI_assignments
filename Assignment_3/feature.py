...
class TfIdfFeatureExtractor(object):
    ...
    def preprocess(self, documents):
        ...
        # Check frequency of each word and add to 'remove_words'
        for w,c in total_word_freq.items():
            if c < self.min_freq:
                remove_words.add(w)
        ...
        # 'doc_word_freq'.
        for word in remove_words:
            temp = total_word_freq.pop(word,0)
            temp = doc_word_freq.pop(word,0)
        ...
        # 'word_freq_tensor'.
        for i,w in enumerate(doc_word_freq.keys()):
            self.word_to_idx[w] = i
        for i,w in enumerate(doc_word_freq.keys()):
            self.idx_to_word[i] = w
        for word,count in doc_word_freq.items():
            word_freq_tensor[self.word_to_idx[word]] = num_docs/count
        # Calculate 'self.idf' (see hint.pdf for formula)
        self.idf = word_freq_tensor
        ''' END YOUR CODE HERE '''
