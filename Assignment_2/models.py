...
class linear_model:
    ...
    def backward(self, y_actl, y_pred, feat):
        ...
        ### YOUR CODE SHOULD BEGIN HERE ###
        no_of_points = np.shape(y_actl)[0]
        self.W = self.W - (self.lr)*(1/no_of_points)*(np.matmul(np.transpose(feat),np.subtract(y_pred,y_actl)))
    ...
    def loss(self, y_actl, y_pred):
        ...
        ### YOUR CODE SHOULD BEGIN HERE ###
        no_of_points = np.shape(y_actl)[0]
        scalar_LOSS = (1/(2*no_of_points))*np.sum(np.square(y_pred-y_actl),axis=0)
...
def calc_features(X, choice, param=5):
    ...
    for x in X:
        if choice == 'linear':
            ...
            ### YOUR CODE SHOULD BEGIN HERE ###
            feat.append(np.append(x,np.ones(1)))
            #pass
            ...
            for j in range(1,param+1):
                row.append(np.sin(j*x))
                row.append(np.cos(j*x))
            row.append(np.ones(1))
            feat.append(np.array(row)) 
            #pass
    ...
    feat = np.array(feat)
    feat = feat.reshape(np.shape(feat)[:2])
